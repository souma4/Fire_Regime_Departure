...
df_1 <- read_csv("C:/Users/JChandler/Desktop/Work/Masters/data/test_data/CBI_classification/Hammer_Creek_Fire_CBI.csv")
df_2 <- read_csv("C:/Users/JChandler/Desktop/Work/Masters/data/test_data/CBI_classification/GILA_Miller_Fire_CBI.csv")
...
threshold_actual_lm_maxkap <- x[match( unique(curve_class),curve_class)][2]
...
lm_curve$des <- as.factor("lm")
mh_curve$des <- as.factor("mh")
curve <- rbind(lm_curve, mh_curve) %>%
  filter(x <= 3 & x > 0)
...
thresh <- "Threshold"
thresholds <- tibble(x = x, y = y, des = des, thresh = thresh)
df_mutate
...
multinomial_curves <- tibble(x = lm_curve$x, Low = 1-lm_curve$y, High = mh_curve$y) %>%
  mutate(Mixed = 1-Low-High) %>%
  filter(x > 0 & x<3)
...
ggplot()+
  ...
  geom_vline(aes(xintercept = x, color = des), thresholds, size =1.2, linetype = "dashed")+
  ...
  geom_line(aes(x=x, y = Low*100), multinomial_curves, size = 1.2, color = gradientPalette[1])+
...
for(i in 1:10){
  ...
  curve_class <- foreach(j =1:length(preds),
                         .combine = c("c")) %do%{
                           class <- curve_classifier(preds[j])
                           return(class)
                         }
  result_lm[i] <- sum(curve_class == testData$class_lm)/length(curve_class)
}
