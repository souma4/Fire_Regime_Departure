pkgs <- c("foreach", "doParallel","sampling","tidyverse", "terra", "sf",  "units", "RColorBrewer","ggpattern")
...
Calculate_fire_regime_and_departure <- function(bps_rast_path,
                                                ...
                                                buffer_tune = -150,
                                                colors2 = RColorBrewer::brewer.pal(3, "RdYlBu")[c(3,1)]){
...
stored_data <- foreach(i = 1:length(mask_units), .export = c("mask_all","fire_path_folder",
 ...
 invisible(lapply(source_paths,source))
   ...
   rm(bps_mask, freq_map,mosaic_stack_30m)
    ...
    frp_contemporary <- vector(length = n.iter)
    frp_historical <- vector(length = n.iter)
    frp_contemporary_var <- vector(length = n.iter)
    frp_historical_var <- vector(length = n.iter)
    ...
    for(k in 1:n.iter){
      ...
      create_severity_distributions(historical_sample,
                                    ...
                                    k)
      emd_freq[k] <- create_emd_hungarian_algo(contemporary_freq,historical_freq, freq = T , normalized = T)
      emd_sev[k] <-  create_emd_hungarian_algo(contemporary_sev,historical_sev, freq = F , normalized = T)
      ...
      frp_contemporary[k] <- year_range/mean(contemporary_freq$freq)
      frp_historical[k] <- year_range/mean(historical_freq$freq)
    ...
    frp_contemporary_med <- quantile(frp_contemporary, .5, names = F, na.rm = T)
    frp_historical_med <- quantile(frp_historical, .5, names = F, na.rm = T)
    
    pfrid <-calculate_pfrid(frp_contemporary_med,frp_historical_med)
    
    frp_contemporary_var <- var(frp_contemporary, na.rm = T)
    frp_historical_var <- var(frp_historical, na.rm = T)
    ...
    stats <- c(
      ...
      proportion_burned, 
      proportion_FRGI_med,
      frp_contemporary_med, 
      frp_historical_med,
      pfrid, 
      frp_contemporary_var, 
      frp_historical_var,
    ...
    stat_df <- list(stat_df)
    
    
    mean_freq <- year_range/c(frp_historical_med,frp_contemporary_med)
    
    
    freq_compare_bar <- ggplot()+
      geom_col_pattern(data = freq_dat, aes(x = freq,y = relfreq, fill = time, pattern = time),
                       pattern_fill = "black",pattern_size = .15,pattern_density = .1,
                       position = "dodge") +
    ...
    sev_density <- ggplot(combined_sev, aes(x = sev, fill = time)) +
      geom_density_pattern( aes(pattern = time),pattern_fill = "black",pattern_size = .15,pattern_density = .1, alpha =.6)+
    ...
    ggsave(filename = paste0(figure_folder,"/Severity_Density_",dir_name,".jpg"), sev_density, width = 1280, height = 720, units = "px")
     ...
     freq_sev_each <- ggplot(combined_sev_class, aes(x=sev, y = relfreq, fill = time))+
      geom_col_pattern(aes(pattern = time),pattern_fill = "black",pattern_size = .15,pattern_density = .1, position = "dodge")+
    ...
    freq_sev_lm2h <- combined_sev_class %>%
      mutate(sev = recode(sev, Low = "Low/Mixed", Mixed = "Low/Mixed")) %>%
      ggplot(aes(x = sev, y = relfreq, fill = time) ) +
      geom_col_pattern(aes(pattern = time), pattern_fill = "black",pattern_size = .15,pattern_density = .1,position = "dodge") +
    ...
    individual_stats <- list(stat_df,freq_compare_bar,sev_density,freq_sev_each,freq_sev_lm2h)
    save(individual_stats, file = paste0(output_name,"/",name_unit,"_stats.RData"))
    gc()
      return(c(stat_df))
        }
